    ...
    class Bsb4Design
    {
        ...
        public function displayComments($comment, $args, $depth)
        {
            if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
                ...
                echo '<div class="comment-author-value">';
                /* translators: $s: Comment author name with link. */
                printf(__('%s', 'bootstrap-basic4'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link()));
                echo '</div>';
                echo PHP_EOL;
                ...
                printf(_x('%1$s à %2$s', '1: date, 2: time', 'bootstrap-basic4'), get_comment_date(), get_comment_time());
                echo '</time>';
                echo '</a>';
                // end date-time
                ...
                echo '<div class="comment-description">';
                comment_text();
                echo '</div>';
                // end comment content body

                // reply link
                echo '<div class="commentaire-btn">';
                comment_reply_link(array_merge($args, [
                    'add_below' => 'div-comment',
                    'depth'     => $depth,
                    'max_depth' => $args['max_depth'],
                    'reply_text' =>  __('Répondre', 'bootstrap-basic4'),
                    'login_text' =>  __('Log in to Reply', 'bootstrap-basic4')
                ]));


                edit_comment_link( __('Modifier', 'bootstrap-basic4'));
                echo '</div>';
